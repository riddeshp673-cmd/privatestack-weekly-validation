...
def kv_request(command, *args):
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            print(f"DEBUG KV raw response: {repr(response_data)}")  # Debug line
            if not response_data:
                return None
            result = json.loads(response_data)
            print(f"DEBUG KV parsed result: {result}")  # Debug line
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            # Test what KV returns for GET on a non-existent key
            test_key = "nonexistent:test:key"
            kv_result = kv_request("GET", test_key)
            ...
            self.wfile.write(json.dumps({
                "test_key": test_key,
                "kv_result": kv_result,
                "kv_result_type": str(type(kv_result)),
                "kv_result_is_none": kv_result is None,
                "kv_result_bool": bool(kv_result)
            }).encode())
